...
interface SeoEntry { id?: string; section: string; locale: string; content: string }
interface SeoContent { metaTitle?: string; metaDescription?: string; ogTitle?: string; ogDescription?: string; canonical?: string }
...
export function SeoManager({ initialData }: { initialData: SeoEntry[] }) {
  ...
  const updateField = (field: keyof SeoContent, value: string) => {
    ...
    setData((prev) => {
      ...
      if (existing) {
        return prev.map((d) => d.section === currentKey && d.locale === activeLocale ? { ...d, content: json } : d)
      }
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/admin/seo', {
        ...
        body: JSON.stringify({ section: currentKey, locale: activeLocale, content: JSON.stringify(parsed) }),
      })
  ...
  const pages = [
    { id: 'home', label: t.homePage },
    ...TOOLS.map((slug) => ({ id: slug, label: slug.split('-').map((w) => w[0].toUpperCase() + w.slice(1)).join(' ') })),
  ]

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">{t.seoManagement}</h1>
      ...
      <div className="flex flex-wrap gap-2">
        {pages.map((p) => (
          <button key={p.id} onClick={() => setActiveTool(p.id)}
            className={`rounded-lg px-3 py-1.5 text-sm transition-colors ${activeTool === p.id ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground hover:bg-accent'}`}>
            {p.label}
          </button>
        ))}
      </div>
      ...
      <div className="max-w-2xl rounded-xl border bg-card p-6 space-y-5">
        ...
        <div className="space-y-2">
          ...
          <Input value={parsed.metaTitle || ''} onChange={(e) => updateField('metaTitle', e.target.value)} />
        </div>
        <div className="space-y-2">
          <Label>{t.metaDescription}</Label>
          <Input value={parsed.metaDescription || ''} onChange={(e) => updateField('metaDescription', e.target.value)} />
        </div>
        <div className="space-y-2">
          <Label>{t.ogTitle}</Label>
          <Input value={parsed.ogTitle || ''} onChange={(e) => updateField('ogTitle', e.target.value)} />
        </div>
        <div className="space-y-2">
          <Label>{t.ogDescription}</Label>
          <Input value={parsed.ogDescription || ''} onChange={(e) => updateField('ogDescription', e.target.value)} />
        </div>
        <div className="space-y-2">
          <Label>{t.canonicalUrl}</Label>
          <Input value={parsed.canonical || ''} onChange={(e) => updateField('canonical', e.target.value)} />
        </div>
      </div>
