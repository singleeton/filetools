...
export function CategoryCard({ lang, meta, label, tagline, countLabel, comingSoon, comingSoonLabel, index = 0 }: CategoryCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden
        className={`absolute -top-8 -left-8 -z-10 h-28 w-28 rounded-full bg-gradient-to-br ${meta.gradient} opacity-25 blur-2xl transition-opacity duration-300 group-hover:opacity-40 dark:opacity-30 dark:group-hover:opacity-45`}
      />
      <div
